...
class GaussianModel:
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        """
        针对在视图空间中具有较大位置梯度（即超过特定阈值）的高斯。其包括在未充分重建(协方差较小)的区域克隆小高斯或在过度重建(协方差较大)的区域分裂大高斯
        未充分重建实际使用的判断方式是scaling较小，而过度重建实际使用的判断方式是scaling较大。判断使用的阈值是percent_dense*extent(两个相机最大距离)
        """
